...
document.getElementById('formulario').addEventListener('reset', () => {
    // Selecciona todos los elementos que muestran errores
    const mensajesError = document.querySelectorAll('[id^="error"]');
    
    // Recorre cada mensaje y lo oculta
...
function validarFormulario() {
    ...
    if (!/^\d{8}$/.test(documento.value)) {
        mostrarError(errorDocumento, '¡El documento debe tener exactamente 8 números!');
        validacionCorrecta = false;
    } else {
